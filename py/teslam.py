...
keywords = "테슬라"
while True:
    urls = "https://search.naver.com/search.naver?where=news&query={}&sm=tab_srt&sort=1&photo=0&field=0&reporter_article=&pd=0&ds=&de=&docid=&nso=so%3Add%2Cp%3Aall%2Ca%3Aall&mynews=0&refresh_start=0&related=0".format(
        keywords)
    err_log = "{}.log".format(news_names)  # 에러로그 저장 이름
    save_news = "{}.txt".format(news_names)
    time.sleep(60)
    ...
    try:
        if tesla_youtue:
            ...
            search = SearchVideos(keywords, offset=1,
